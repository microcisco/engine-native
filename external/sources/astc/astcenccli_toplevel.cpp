...
//	double start_time = get_time();
	...
	int error = 0;
	astcenc_operation operation = ASTCENC_OP_DECOMPRESS;
	astcenc_profile profile = ASTCENC_PRF_LDR;



	std::string input_filename = "a.astc";
	std::string output_filename = "a.png";
	...
	if (operation & ASTCENC_STAGE_LD_COMP)
	{
		...
		{
			error = load_cimage(input_filename.c_str(), image_comp, data, dataLen);
			if (error)
			{
				return r;
			}
	...
	if (operation & ASTCENC_STAGE_DECOMPRESS)
	{
		...
		{
			double start_iter_time = get_time();
			if (false && cli_config.thread_count > 1)
			{
   ...
   // Print metrics in comparison mode
	...
	int len;
	unsigned char *png = stbi_write_png_to_mem((const unsigned char *)image_decomp_out->data[0], image_decomp_out->dim_x * 4,
		image_decomp_out->dim_x, image_decomp_out->dim_y,
		4, &len);
